...
class Game {
  ...
  levelRender() { //premade level render
    ...
    if (this.gameStateID === 4) { //level 1
    ...
    } else if(towerGame.gameStateID === 8){
      //left part of map
      // for()
      ...
      for (let i = 0; i < level5.length; i++) {
        if (towerGame.placingTower && towerGame.canAddTower(level5[i])) {
          towerGame.placeTower(level5[i]);
        }
        else if (!towerGame.placingTower && !level5[i].hasTower) {
          // toggle the occupied property of the clicked cell
          level5[i].occupied = true;

          towerGame.brushfire(towerGame.undo(level5[i]));
        }
      }
    }
  }
